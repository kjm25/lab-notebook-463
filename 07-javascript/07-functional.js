...
const mapNameFamily = (characters) => {
  ...
  result = characters.map(
    (charData) => `${charData.name}, of House ${charData.house}}`
  );
...
const filterFamily = (characters, house) => {
  ...
  result = characters.filter((charData) => charData.house === house);
...
const reduceHouses = (characters) => {
  ...
  const result = characters.reduce((total, charData) => {
    if (total.hasOwnProperty(charData.house))
      total[charData.house]++; //if house exists in total add one
    else total[charData.house] = 1; // if house doesn't exist add new property with starting at 1
    return total;
  }, {});
